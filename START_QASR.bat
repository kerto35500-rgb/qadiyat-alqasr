...
rem ---- sanity check --------------------------------------------------
if not exist "%~dp0serve_qasr.py" goto no_server
if not exist "%~dp0index.html" goto no_page

rem ---- free the port if an old run is still listening -----------------
for /f "tokens=5" %%P in ('netstat -ano ^| findstr /r /c:"TCP .*:8010 .*LISTENING"') do taskkill /F /PID %%P >nul 2>nul

echo.
echo  ============================================================
echo    Qadiyat Al-Qasr   ^(qasr build^)
echo    http://localhost:8010/qasr/index.html
...
:no_server
echo.
echo  [X] serve_qasr.py is missing next to this file:
echo      %~dp0serve_qasr.py
echo.
popd
pause
exit /b 1
